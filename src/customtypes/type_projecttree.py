    ...
    def __init__(self, **kwargs):
        ...
        for key in ['files', 'file']:
            if key in kwargs and (kwargs[key] is None or isinstance(kwargs[key], list)):
                files = kwargs[key] or [];
                break;
        assert all(isinstance(file, str) for file in files), 'Component \033[1mfiles:\033[0m must be a list of strings.';
        directories = dict();
        ...
        self._directories = { path: ProjectTree(**struct) for path, struct in directories.items() };
        return;
